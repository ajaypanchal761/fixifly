...
async function fixWalletCredit() {
  try {
    console.log('🔧 Fixing wallet credit for ticket TK000004...');
    ...
    // Get vendor details to find the vendor ID string
    ...
    // Get vendor wallet using the vendor ID string
    const vendorWallet = await VendorWallet.findOne({ vendorId: vendor.vendorId });
    if (!vendorWallet) {
      console.log('❌ Vendor wallet not found for vendor ID:', vendor.vendorId);
      return;
    }
    ...
    if (ticket.completionData) {
      ...
      const updatedWallet = await VendorWallet.findOne({ vendorId: vendor.vendorId });
    ...
    console.error('❌ Error fixing wallet credit:', error);
  } finally {
